...
//        "QIMEI36": "%s",
    ...
    public  String searchRequestParam(String query,String search_type ,Integer page_num,Integer num_per_page) {
     ...
     String   msg = """
        ...
        // QQ 音乐接口对 JSON 字段名非常严格；同时对搜索词做 JSON 转义，避免引号或反斜杠导致整段请求失效。
        String safeQuery = JSON.toJSONString(query == null ? "" : query);
        String format = String.format(msg, search_type, safeQuery,
                page_num == null || page_num < 1 ? 1 : page_num,
                num_per_page == null || num_per_page < 1 ? 20 : Math.min(num_per_page, 50));
        return format;
    }
